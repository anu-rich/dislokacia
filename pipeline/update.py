...
def kind(f):
    ...
    if "crude oil" in n: return "crude"
    if "own fleet" in n: return "openseas"
    if n.endswith((".pptx", ".pdf")) and "мр" in n: return "mr"
    if "дислокац" in n and n.endswith((".xlsx", ".xlsm", ".xls")): return "disl"
    return "?"
crude_files = [f for f in allf if kind(f) == "crude"]
os_files = [f for f in allf if kind(f) == "openseas"]
mr_files = [f for f in allf if kind(f) == "mr"]
files = sorted([f for f in allf if kind(f) == "disl"], key=lambda f: os.path.basename(f))
if crude_files: subprocess.run([sys.executable, "parse_crude.py", "data/crude.csv"] + crude_files, check=True)
if os_files: subprocess.run([sys.executable, "parse_openseas.py", "data/openseas.csv"] + os_files, check=True)
if mr_files: subprocess.run([sys.executable, "parse_mr.py", "data/mr.json"] + mr_files, check=True)
if not files:
    if not os.path.exists("data/snapshot.json"):
        sys.exit("укажи хотя бы один xlsx файл дислокации")
    print("файлов дислокации нет — пересборка по имеющимся данным")
